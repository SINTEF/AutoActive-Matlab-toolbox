function info = ffmpeginfoVideoSync(infile)
...
if isempty(str)
   ...
   rows = size(data);
   for row = 1:rows
     cell_name = char(data(row,1));
     if contains(cell_name,'-') == 1 | contains(cell_name,'.') == 1 
         cell_name = strrep(cell_name,'-','_');
         cell_name = strrep(cell_name,'.','_');
         data(row,1) = cellstr([cell_name]);
     end
   end
